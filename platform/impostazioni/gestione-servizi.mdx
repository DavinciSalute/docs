...
### 7b. Invio PDF dei certificati tramite e-mail

- L'invio dei **certificati** in formato PDF tramite e-mail è **sempre abilitato** per tutti i piani: non è necessario attivare alcuna impostazione.
- Quando viene creato un certificato, il PDF viene automaticamente inviato al paziente via e-mail secondo le impostazioni di modalità di invio configurate (invio in chiaro o invio protetto).

<Info>
  A differenza delle ricette (sezione 7), per le quali è necessario abilitare manualmente l'invio via e-mail, i certificati vengono inviati automaticamente via e-mail senza alcuna configurazione aggiuntiva.
</Info>
