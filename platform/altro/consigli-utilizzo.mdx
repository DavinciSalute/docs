---
title: "Consigli di utilizzo"
sidebarTitle: "Consigli di utilizzo"
---

### 1. Icona di Elty DaVinci sul Desktop

- Fare il login tramite [https://davinci.elty.it/](https://davinci.elty.it/)
- Aggiungere il collegamento a Elty su Desktop
  - Chrome → Tre puntini in alto a destra → Trasmetti, salva e condividi → Crea scorciatoia

<img
  src="/images/platform/consigli-utilizzo/Screenshot_2025-03-04_alle_17.51.59.png"
  alt="Screenshot 2025-03-04 alle 17.51.59.png"
  className="mx-auto"
/>

### 2. Informazioni sulle esenzioni

Le esenzioni da reddito consentono al cittadino di ottenere l’esonero totale o parziale dal pagamento dei ticket, cioè gli importi che vengono richiesti dal Servizio Sanitario Nazionale (SSN) o regionale (SSR) a titolo di compartecipazione alla spesa sanitaria.

Le quattro tipologie di esenzioni da reddito a livello nazionale sono:

- E01 - per età inferiori a 6 anni o superiori a 65 anni con reddito familiare inferiore a 36.151,98 euro;
- E02 – disoccupati, e loro familiari a carico, con reddito familiare inferiore a 8.263,31 euro incrementato a 11.362,05 euro in presenza del coniuge e di ulteriori 516,46 euro per ogni figlio a carico;
- E03 - titolari di assegno (ex pensione) sociale, e loro familiari a carico;
- E04 - titolari di pensione al minimo, con più di 60 anni, e loro familiari a carico, con reddito familiare inferiore a 8.263,31 euro incrementato a 11.362,05 euro in presenza del coniuge e di ulteriori 516,46 euro per ogni figlio.

La 01 e la 048 sono accettate fino al 22/03/2026, in seguito è in vigore la N01. Per maggiori informazioni [https://www.aifa.gov.it/nota-01](https://www.aifa.gov.it/nota-01)

- È obbligatoria la modifica manuale della nota (per ogni farmaco al momento dell'elaborazione) perché ci sono delle differenze di contenuto tra le note 01, 048 e N01, per cui la presa visione da parte del medico è necessaria; aggiornando i farmaci massivamente rischieremmo che qualche medico non si accorga della modifica o che per qualche prescrizione nello specifico non siano fatte le dovute valutazioni cliniche.

### 3. Informazioni sui profili

- L’unica tipologia di profilo che può elaborare le ricette è il profilo medico. Se un profilo di segreteria o infermeria ha errori come “Qualcosa è andato storto”, si possono escludere tutte le motivazioni legate alla prescrizione e alla connessione con il sistema regionale.
- Infermeria, segreteria e prenotatori non si connettono mai al sistema regionale.

### 4. Gestione degli informatori scientifici / farmaceutici

- Attualmente gli informatori non possono registrarsi come "Informatore" ma ci sono 3 metodi per gestire le loro prenotazioni:

1. Prenotazioni da parte dell'informatore che scarica l'app:

- L'informatore scarica l'app come se fosse un paziente
- Può cambiare medico associato per prenotare appuntamenti
- Il medico può inserire degli slot orari visibili ai pazienti chiamati "Informatore scientifico"
- Il limite di questa opzione è che altri pazienti potrebbero prendere appuntamenti in questi slot (ma normalmente non accade, non scelgono la prestazione "Informatore scientifico" e nel caso questi appuntamenti possono essere eliminati)

2. Visibilità web delle disponibilità:

- Su richiesta possiamo creare una pagina web collegata all'agenda dove mostrare solo le disponibilità orarie per gli informatori
- Il link può essere fornito agli informatori stessi che possono prenotarsi in autonomia
- Le disponibilità modificate su Elty impiegano 24 ore ad essere modificate anche sul sito web

3. Inserimento di eventi all'interno dell'agenda (chiamati "Informatore scientifico"):

- In questo caso si ha l'appuntamento telefonico da parte dell'informatore
- Semplicemente si blocca lo slot orario in agenda così da evitare sovrapposizioni con altri appuntamenti

### 5. Visualizzare la schermata di Home

- Per visualizzare la pagina principale (**home**) ci sono due metodi:
  - Vai al link [davinci.elty.it](http://davinci.elty.it)
  - Clicca il logo Elty Davinci in alto a sinistra da qualsiasi sezione della webapp

### 6. Non trovo un farmaco nella lista di Elty

- I farmaci all'interno del database di Elty dipendono da Farmadati e sono tutti i farmaci autorizzati dall'AIFA.
- Se non trovi un farmaco e vuoi esser sicuro che sia effettivamente prescrivibile, puoi cercarlo dal sito ufficiale dell'AIFA: [https://medicinali.aifa.gov.it/it/#/it/](https://medicinali.aifa.gov.it/it/#/it/)

### 7. Cancellare la cache del browser

- A volte per risolvere alcune problematiche di visualizzazione della webapp bisogna **cancellare la cache del browser**.
- I passaggi per svuotare la cache del browser **Google Chrome** sono:
  - Clicca sui **tre puntini** verticali in alto a destra.
  - Seleziona **"Cronologia"** e poi di nuovo **"Cronologia"**.
  - Clicca su **"Elimina dati di navigazione"** (sulla sinistra).
  - Seleziona solo **"Immagini e file memorizzati nella cache"**.
  - Clicca su **"Elimina dati"**.
  - Chiudi e riapri il browser.
- Accedendo di nuovo ad Elty verranno richieste le credenziali, se non si ricorda la password puoi cliccare **"Password dimenticata"** e procedere al reset password.

### 8. Gli orari (disponibilità) dell'agenda risultano spostati di un'ora da un giorno all'altro

- Questo accade quando le impostazioni **"Data/ora e lingua"** del sistema operativo non sono state impostate correttamente, gli orari delle disponibilità in agenda risultano spostati di un'ora nel momento in cui cambia l'ora legale:
  <img
    src="/images/Screenshot2026-02-11alle12.34.29-1.png"
    alt="Screenshot 2026 02 11 Alle 12 34 29"
    title="Screenshot 2026 02 11 Alle 12 34 29"
    className="mx-auto"
  />
- Vai nelle impostazioni di Windows, clicca **"Data/ora e lingua"**.
- Deseleziona **"Imposta il fuso orario automaticamente"**.
- Controlla che il Fuso orario sia **UTC \+ 01:00** Amsterdam, Berlino, Roma ...
- Clicca **"Passa automaticamente all'ora legale"**.
  <img
    src="/images/Screenshot2026-02-11alle12.37.51.png"
    alt="Screenshot 2026 02 11 Alle 12 37 51"
    title="Screenshot 2026 02 11 Alle 12 37 51"
    className="mx-auto"
  />